  ...
  try {
    ...
    try {
      ...
      if (existingLoc.rows.length === 0) {
        await client.query(
          `INSERT INTO tenant_locations (id, tenant_id, name, timezone, status, is_primary, latitude, longitude, address, created_at, updated_at)
           VALUES ($1, $2, 'Local Principal', 'America/Argentina/Buenos_Aires', 'active', true, $3, $4, $5, now(), now())`,
          [randomUUID(), tenantId, "-37.107500", "-56.861400", JSON.stringify({ lat: -37.1075, lng: -56.8614, formattedAddress: "Pinamar, Buenos Aires, Argentina", geocoderProvider: "photon" })],
        );
      } else {
        await client.query(
          `UPDATE tenant_locations SET latitude = COALESCE(latitude, $2), longitude = COALESCE(longitude, $3), address = COALESCE(address, $4), updated_at = now() WHERE id = $1`,
          [existingLoc.rows[0].id, "-37.107500", "-56.861400", JSON.stringify({ lat: -37.1075, lng: -56.8614, formattedAddress: "Pinamar, Buenos Aires, Argentina", geocoderProvider: "photon" })],
        );
      }
