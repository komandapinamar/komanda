...
type RouteContext = { params: Promise<{ tenantId: string; shiftId: string }> };

export async function POST(request: Request, route: RouteContext) {
  const correlationId = correlationIdFromRequest(request);
  try {
    const { tenantId, shiftId } = await route.params;
    const context = await administrativeTenantContext(request, tenantId, correlationId);
    requireOwnerOrAdmin(context);

    const body = await request.json();
    const result = await new CashShiftService().close(context, shiftId, body);
    return Response.json(result);
  } catch (error) {
    if (error instanceof CashShiftNotFoundError) {
      return Response.json({ title: error.message }, { status: 404 });
    }
