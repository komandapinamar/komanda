...
**Tests**: Tests are mandatory for cross-repository contracts, tenant isolation,
critical registration/checkout/payment/order/print flows, and other cases required by
the constitution. For other work, include tests when requested by the specification.
...
- [ ] T010 Define or update versioned producer/consumer contracts when crossing repository or client boundaries
- [ ] T011 Add explicit tenant-context enforcement and two-tenant isolation fixtures when tenant data is affected
- [ ] T012 Define timeout, retry, idempotency, and degraded-mode handling for cross-boundary side effects
- [ ] T013 Add correlation and safe tenant diagnostics for affected critical flows
...
- **Contract rollout**: Producer changes MUST remain compatible until all consumers migrate; document deployment and retirement order
...
- Tests required by the constitution or specification MUST be included and pass before story completion
...
- Verify repository ownership, contract compatibility, tenant isolation, and degraded mode before release
